        ...
        return (
          <div key={idx} className={`max-w-[390px] sm:w-[45%] w-full`}>
            ...
            >
              ...
              <p className={`${style.p} !text-[20px] font-semibold`}>{item.contacts}</p>
            </Link>
          </div>
        );
      })}
      <hr className="w-full border-t-2px border-gray-400"/>
      <p className={`${style.p} text-center !text-gray-800`}>© 2006-2025 Greencards.uz. Barcha huquqlar himoyalangan. Shaxsiy ma’lumotlaringiz boshqa shaxslarga taqdim etilmaydi.</p>
    </div>
  );
};
